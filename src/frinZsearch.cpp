...
std::vector<std::vector<float>> generate_shifted_amplitude_plane(
    ...
    if (N_cols_padded_fft_dim > 0) {
        ...
        if (N_rows_original_data > 0 && N_cols_padded_fft_dim > 0) { 
            scale_factor_horizontal_ifft = 1 / (effective_integ_length * N_rows_original_data); 
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < rows_to_copy_to_fft_in; ++i) { // コピーする行数を N_rows_padded 以下に制限
        for (int j = 0; j < cols_to_copy_to_fft_in; ++j) { 
            ...
            int buffer_index = i * N_cols_fft + j; // fft_in のインデックス

            if (is_rfi_channel) {
                fft_in[buffer_index][0] = 0.0f;
                fft_in[buffer_index][1] = 0.0f;
            } else {
                fft_in[buffer_index][0] = current_all_spectrums[static_cast<size_t>(i)][static_cast<size_t>(j)].real();
                fft_in[buffer_index][1] = current_all_spectrums[static_cast<size_t>(i)][static_cast<size_t>(j)].imag();
            }

            // 周波数軸のDC成分(j=0、つまり各時間サンプルのスペクトルの0Hz成分)をカット
            // RFI処理の後に行うことで、RFIで既に0にされていても問題ありません。
            if (j == 0) {
                fft_in[buffer_index][0] = 0.0f;
                fft_in[buffer_index][1] = 0.0f;
            }
    ...
    const float norm_factor = 1 / (current_segment_num_sectors * first_effective_integration_length); //static_cast<float>(loaded_data.header.fft_point) / static_cast<float>(N_rows_padded * N_cols_fft);
